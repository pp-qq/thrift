...
public:
  ...
  void run() {
    try {
          processor_->process(input_, output_, nullptr);
    } catch (const std::exception &e) {
          GlobalOutput.printf("TNonblockingServer: client died: %s", e.what());
          connection_->close();
    } catch (...) {
          GlobalOutput.printf("TNonblockingServer: client died: UNKNOWN EXCEPTION");
          connection_->close();
    }
  }
